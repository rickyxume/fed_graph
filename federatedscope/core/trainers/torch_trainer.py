...
class GeneralTorchTrainer(Trainer):
    ...
    def update(self, model_parameters, strict=False):
        ...
        local_model_state_dict = copy.deepcopy(self.ctx.model.state_dict())
        server_model_state_dict = self._param_filter(model_parameters)
        for key in server_model_state_dict.keys():
            try:
                local_model_state_dict[key] = local_model_state_dict[key] * 0.0 + server_model_state_dict[key] * 1.0
            except:
                pass
        self.ctx.model.load_state_dict(local_model_state_dict,
                                       strict=strict)
    ...
    def register_default_hooks_train(self):
        ...
        self.register_hook_in_train(self._hook_on_epoch_end, "on_epoch_end")
    ...
    def register_default_hooks_eval(self):
        ...
        self.register_hook_in_eval(self._hook_on_epoch_end, "on_epoch_end")
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        if ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
            ...
            if ctx.optimizer is None: # 避免重新定义 optimizer
                ctx.optimizer = get_optimizer(ctx.model, **ctx.cfg[ctx.cur_mode].optimizer)
                # TODO: support more types of lr_scheduler
                if self.cfg.train.lr_scheduler.type == 'ExponentialLR':
                    ctx.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(ctx.optimizer, gamma=self.cfg.train.lr_scheduler.gamma)
                elif self.cfg.train.lr_scheduler.type == 'StepLR':
                    ctx.lr_scheduler = torch.optim.lr_scheduler.StepLR(ctx.optimizer, step_size=self.cfg.train.lr_scheduler.step_size, gamma=self.cfg.train.lr_scheduler.gamma)
                elif self.cfg.train.lr_scheduler.type == 'ReduceLROnPlateau':
                    ctx.lr_scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(ctx.optimizer, mode=self.cfg.train.lr_scheduler.mode, factor=self.cfg.train.lr_scheduler.factor, patience=self.cfg.train.lr_scheduler.patience, verbose=True, min_lr=self.cfg.train.lr_scheduler.min_lr)
                else:
    ...
    def _hook_on_epoch_start(self, ctx):
        ...
        if ctx.get("{}_loader".format(ctx.cur_data_split)) is None:
            cur_data = ctx.get("{}_data".format(ctx.cur_data_split))
            loader = get_dataloader(WrapDataset(cur_data),self.cfg)
        ...
        # 监控学习率变化
        if ctx.cfg.wandb.use:
            wandb.log({f"learning rate": ctx.optimizer.param_groups[0]["lr"]})
    ...
    def _hook_on_epoch_end(self, ctx):
        ...
        if self.cfg.train.lr_scheduler.type == 'ExponentialLR':
            ...
            if ctx.cur_data_split == 'val':
                val_loss = ctx.get("loss_batch_total_val")
                # wandb.log({f"loss_batch_total_{ctx.cur_data_split}": val_loss})
                ctx.lr_scheduler.step(val_loss)
            pass    
        else:
            raise NotImplementedError
