...
class Client(Worker):
    ...
    def _register_default_handlers(self):
        ...
        self.register_handlers('evaluate_best', self.callback_funcs_for_evaluate_best)
    ...
    def callback_funcs_for_evaluate(self, message: Message):
        ...
        if self.early_stopper.early_stopped and self._cfg.federate.method in [
            ...
            if self.best_results[f"client #{self.ID}"][self._cfg.eval.best_res_update_round_wise_key] == \
                    formatted_eval_res['Results_raw'][self._cfg.eval.best_res_update_round_wise_key]:
                self.best_model = copy.deepcopy(self.trainer.ctx.model)
    ...
    def callback_funcs_for_evaluate_best(self, message: Message):
        """
        The handling function for receiving the request of evaluating
        ...
        sender = message.sender
        self.state = message.state
        self.trainer.ctx.model.load_state_dict(self.best_model.state_dict())
        # if message.content is not None:
        #     self.trainer.update(message.content,
        #                         strict=self._cfg.federate.share_local_model)
        if self.early_stopper.early_stopped and self._cfg.federate.method in [
